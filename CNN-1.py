...
def convnet(input):
    ...
    h_conv3_flat = tf.reshape(h_norm1, [-1, final_height*final_width*num_hidden_1])
...
def training_output(x):
    ...
    image_list = tf.unstack(x_image, mini_batch_size)
    ...
    x_preprocessed = tf.stack(preprocessed_image_list)
...
# get cifar10.whitened_image.npy
if os.path.exists('../../data/cifar10/cifar10.whitened_image.npy') == False:
    get_whitened_image()
...
final_height = int(cropped_height / 2)
final_width = int(cropped_width / 2)
W_fc2 = weight_variable([final_height*final_width*num_hidden_1, 10])
...
cross_entropy = -tf.reduce_sum(y_*tf.log(y_train)) + reg * (tf.reduce_mean(tf.abs(W_conv1)) + tf.reduce_mean(tf.abs(W_fc2)))
...
for i in range(200000):
    ...
    if (i+1)%1000 == 0:
        print("%d\t%g" % (i+1,test_error.eval(feed_dict={x: data.test.images, y_: data.test.labels})))
    train_step.run(session=sess, feed_dict={x: batch[0], y_: batch[1]})

print("%g" % test_error.eval(feed_dict={x: data.test.images, y_: data.test.labels}))
